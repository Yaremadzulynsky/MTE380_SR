#!/usr/bin/env python3
"""
Perception tuner — runs on any machine with OpenCV, no Pi camera needed.
...
Usage:
    ...
    # From an image file
    python test_perception.py --source path/to/image.jpg

    # From a video file
    python test_perception.py --source path/to/video.mp4
...
What is shown:
    Grey line    : ROI boundary (only the area below is processed)
    White line   : frame centre (target for zero error)
    Green rect   : bounding box of detected red contour
    Green lines  : left/right tape edges extended to bottom of frame
    Red dot      : centroid the robot steers toward
    Orange line  : error — from frame centre to centroid
    Cyan text    : detection values
    Red mask     : semi-transparent overlay of the HSV red mask
"""
...
def detect(frame: np.ndarray, roi_top_ratio: float, red_min_area: float,
           blue_min_area: float, t_ratio: float):
    h, w  = frame.shape[:2]
    ...
    if contours:
        ...
        if area >= red_min_area:
            ...
            if M["m00"] > 1e-6:
                ...
                red_error = max(-1.0, min(1.0, (cx_px - w / 2.0) / (w / 2.0)))
            # shift cx_px to full-frame coords
            cx_px_full = cx_px
            cy_px_full = roi_y + (by + bh // 2)
        else:
            cx_px_full = w // 2
            cy_px_full = roi_y + (h - roi_y) // 2
    else:
        cx_px_full = w // 2
        cy_px_full = roi_y + (h - roi_y) // 2
    ...
    return {
        ...
        "roi_y":      roi_y,
        "bbox":       bbox,
        "cx_px":      cx_px_full if red_found else None,
        "cy_px":      cy_px_full if red_found else None,
        "red_mask":   red_mask,
        "blue_mask":  blue_mask,
...
def draw(frame: np.ndarray, d: dict) -> np.ndarray:
    out  = frame.copy()
    h, w = out.shape[:2]
    roi_y = d["roi_y"]
    ...
    # All contour areas (small text, helps tune red_min_area)
    for i, (area, (bx, by, bw, bh)) in enumerate(d["all_contours"][:6]):
        fy = roi_y + by
        cv2.putText(out, f"{int(area)}", (bx, fy - 4),
...
def main() -> None:
    ...
    if src.isdigit():
        ...
        is_image = src.lower().endswith((".jpg", ".jpeg", ".png", ".bmp"))

    if is_image:
        frame0 = cv2.imread(src)
        if frame0 is None:
    ...
    while True:
        ...
        # Parameter overlay (bottom of frame)
        h, w = vis.shape[:2]
        cv2.putText(vis,
                    f"roi={roi_ratio:.2f}  red_min_area={red_min_area:.0f}  "
                    f"[/]=ROI  -/==area",
                    (10, h - 10),
