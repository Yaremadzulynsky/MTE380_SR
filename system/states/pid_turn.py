"""
PID_TURN state — triggered when curvature exceeds the configured threshold.
...
def on_enter(sm, det, left_ticks: int, right_ticks: int) -> None:
    """Stash turn-specific state on the SM before the generic _enter() call."""
...
def _step_forward(sm, left_ticks: int, right_ticks: int) -> ControlOutput:
    ...
    if ctrl.done:
        ...
        print(
            f"[pid_turn] forward done  traveled={traveled:.3f}m  target={target:.3f}m",
            flush=True,
        )
...
def _step_rotate(sm, left_ticks: int, right_ticks: int) -> ControlOutput:
    ...
    if ctrl.done:
        traveled, target = ctrl.progress()
        print(
            f"[pid_turn] rotation done  traveled={traveled:.1f}°  target={target:.1f}°",
            flush=True,
        )
